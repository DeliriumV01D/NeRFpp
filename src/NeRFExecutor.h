...
public:
	...
	void Train(NeRFExecutorTrainParams &params)
	{
		...
		if (!params.NoBatching)
		{
			...
			//	rays.emplace_back(torch::stack({ rays_o, rays_d }, 0/*?*/));	//[N, ro + rd, H, W, 3]
			//}
			//std::copy(data.Imgs.begin(), data.Imgs.end(), back_inserter(rays));
			//torch::Tensor rays_rgb = torch::cat(rays, 1);				//[N, ro + rd + rgb, H, W, 3]
			...
			//rays.clear();
			//for (auto i : itrain)			
			//	rays.emplace_back(rays_rgb.index({i}));
			//rays_rgb = torch::stack(rays, 0);			//train images only
		...
		for (int i = this->Start + 1; i < n_iters; i++)
		{
			...
			{ 
				...
				if (params.NRand != 0)
				{
					...
					//в оригинале select_inds = np.random.choice(coords.shape[0], size = [N_rand], replace = False)  # (N_rand, )
					//можно использовать torch.multinomial(input, num_samples, replacement=False, *, generator=None, out=None) → LongTensor
					//но здесь простой случай, кажется
			...
			auto new_lrate = LearningRate * powf(decay_rate, global_step / decay_steps);
